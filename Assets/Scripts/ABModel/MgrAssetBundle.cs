...
namespace SUIFW {
    public class MgrAssetBundle : MonoBehaviour{
        private static MgrAssetBundle m_Instance;
        private Dictionary<string, MultiABMgr> m_DicMultiABMgr;
        private AssetBundleManifest m_ABManifestObj;


        public static MgrAssetBundle GetInstance() {
            if (null == m_Instance) {
                m_Instance = new GameObject("AssetBundleMgr").AddComponent<MgrAssetBundle>();
            }
            return m_Instance;
        }
